...
class MuseDataset(Dataset):
    ...
    def _get_audio_sample_path(self, item):
        path = os.path.join(self.audio_dir, self.annotations.iloc[item]['spotify_id'] + '.mp3')
        return path

    def _get_audio_sample_label(self, item):
        return self.annotations.iloc[item][['valence', 'arousal', 'dominance']].to_numpy()

    def __getitem__(self, item):
        audio_sample_path = self._get_audio_sample_path(item)
        labels = self._get_audio_sample_label(item)
